<template>
  <div
    ...
    v-show="
      search_title
        ? obj.title.toLowerCase().indexOf(search_title.toLowerCase()) != -1
        : tags.length != 0
  ...
  >
    ...
    <div>
      <h1>{{ obj.title }}</h1>
      <div
        v-if="Array.isArray(obj.paragraph)"
        v-html="obj.paragraph.join('<br>')"
      ></div>
      <div
        v-else-if="obj.paragraph"
        v-html="Object.values(obj.paragraph).join('<br>')"
      ></div>
    ...
    <figure v-if="obj.img">
      <svg v-if="obj.img.src.indexOf('#') == 0">
        <use
          :xlink:href="require('~/assets/svg/svg_list.svg') + obj.img.src"
          :alt="obj.img.alt"
        ></use>
      </svg>
      <NuxtImg
